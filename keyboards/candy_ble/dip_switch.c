...
#define NUMBER_OF_DIP_SWITCHES (sizeof(dip_switch_pad) / sizeof(uint8_t))
static uint8_t dip_switch_pad[]                              = DIP_SWITCH_PINS;
...
void dip_switch_init(void) {
    for (uint8_t i = 0; i < NUMBER_OF_DIP_SWITCHES; i++) {
        ...
        nrf_gpio_cfg_input(dip_switch_pad[i],NRF_GPIO_PIN_PULLUP);
    }
...
void dip_switch_read(bool forced) {
    ...
    for (uint8_t i = 0; i < NUMBER_OF_DIP_SWITCHES; i++) {
        ...
        dip_switch_state[i] = !nrf_gpio_pin_read(dip_switch_pad[i]);
